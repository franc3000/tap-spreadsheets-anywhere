...
def get_streamreader(uri, universal_newlines=True, newline="", open_mode="r", encoding="utf-8"):
    ...
    streamreader = smart_open.open(
        uri, open_mode, newline=newline, errors="surrogateescape", encoding=encoding, **kwargs
    )
        ...
        if format == "csv":
            ...
            if uri.lower().endswith(".xls"):
                ...
                reader = get_streamreader(
                    uri, universal_newlines=universal_newlines, newline=None, open_mode="rb", encoding=None
                )
